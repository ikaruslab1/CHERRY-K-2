...
export default function AttendanceView() { // Default export for import ease
    ...
    return (
        <div className="w-full max-w-sm xs:max-w-md md:max-w-lg xl:max-w-xl mx-auto px-1 xs:px-4 py-2 space-y-4 xs:space-y-6 md:space-y-8">
            ...
            <div className="bg-white p-4 xs:p-6 md:p-8 rounded-2xl xs:rounded-3xl shadow-sm border border-gray-200 space-y-3 xs:space-y-4">
                <div>
                    <h2 className="text-base xs:text-xl md:text-2xl font-black text-gray-900">Control de Asistencia</h2>
                    <p className="text-xs xs:text-sm md:text-base text-gray-500 mt-0.5">Escanea el código QR de los asistentes.</p>
                </div>

                <div className="space-y-1.5 xs:space-y-2">
                    <label className="text-[10px] xs:text-xs font-bold uppercase text-gray-500 tracking-wider block">Actividad Actual</label>
                    {loadingActivities ? null : (
                        <div className="relative">
                            <select
                                value={selectedActivity}
                                onChange={(e) => setSelectedActivity(e.target.value)}
                                className="w-full appearance-none bg-gray-50 border border-gray-300 text-gray-900 font-semibold rounded-xl py-2.5 xs:py-3 px-3.5 pr-10 focus:outline-none focus:ring-2 focus:ring-[var(--color-acid)] text-xs xs:text-sm"
                            >
                                <option value="" disabled>Seleccionar Actividad...</option>
                                {activities.map(evt => (
                                    <option key={evt.id} value={evt.id} className="bg-white text-gray-900">{evt.title}</option>
                                ))}
                            </select>
            ...
            {/* Scanner Section */}
            <div className="relative rounded-2xl xs:rounded-3xl overflow-hidden shadow-lg border border-gray-200">
                {!selectedActivity ? (
                    <div className="aspect-square bg-gray-50 flex flex-col items-center justify-center text-center p-6 space-y-3">
                        <div className="w-14 h-14 bg-gray-100 rounded-full flex items-center justify-center">
                            <AlertTriangle className="h-7 w-7 text-amber-500" />
                        </div>
                        <div>
                            <h3 className="text-gray-900 font-bold text-sm xs:text-base">Cámara Desactivada</h3>
                            <p className="text-xs xs:text-sm text-gray-500 mt-1 max-w-xs">Selecciona una actividad arriba para iniciar el escáner.</p>
                        </div>
                    </div>
                ) : (
                    <>
