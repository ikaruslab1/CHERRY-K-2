...
export function AgendaBlock({ block, conferenceId, locale = 'es' }: { block: LandingBlock; conferenceId?: string; locale?: string }) {
  const { title, title_en, activities = [] } = block.content;
    ...
    async function fetchEvents() {
      ...
      try {
        const { data, error } = await supabase
          .from('events')
          .select('id, title, title_en, location, location_en, type, type_en, date')
      ...
      <div className="container mx-auto px-6 max-w-4xl">
        <div className="flex flex-col items-center mb-16 text-center">
           ...
           <h2 className="text-4xl font-bold text-gray-900 tracking-tighter">
             {locale === 'en' ? (title_en || 'Event Schedule') : (title || 'Cronograma del Evento')}
           </h2>
        </div>
          ...
          {events.length > 0 ? (
            <>
              {events.map((e: any, idx: number) => {
                ...
                const timeStr = dateObj.toLocaleTimeString(locale === 'en' ? 'en-US' : 'es-MX', {
                  hour: '2-digit',
                  minute: '2-digit',
                  hour12: false,
                ...
                const displayTitle = locale === 'en' && e.title_en ? e.title_en : e.title;
                const displayType = locale === 'en' && e.type_en ? e.type_en : (e.type || (locale === 'en' ? 'Session' : 'Sesión'));
                const displayLocation = locale === 'en' && e.location_en ? e.location_en : (e.location || (locale === 'en' ? 'Main Room' : 'Sala Principal'));
                ...
                return (
                  ...
                  >
                    <div className="w-24 flex-shrink-0 pt-1 text-center border-r border-gray-200">
                      <p className="text-xs font-bold text-gray-400 uppercase">{locale === 'en' ? 'Start' : 'Inicio'}</p>
                      <p className="text-xl font-black text-gray-900">{timeStr}</p>
                    </div>
              ...
              <div className="mt-8 border border-gray-200 rounded-[2.5rem] overflow-hidden bg-gray-100">
                ...
                >
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 group-hover:text-black transition-colors">
                    {locale === 'en' ? 'Attendance and registration information' : 'Información sobre asistencia e inscripciones'}
                  </span>
                    ...
                    >
                      <div className="px-12 pb-12 space-y-8 text-center border-t border-gray-200/50 pt-8 mt-[-1px]">
                        <p className="text-gray-400 text-[13px] max-w-xl mx-auto leading-relaxed font-medium italic">
                          {locale === 'en' 
                            ? '"To ensure a complete and personalized experience during the event, we invite you to access your account. By logging into our portal, you will be able to consult the detailed breakdown of each session, officially register for the activities of your interest and automatically manage your attendance to obtain your participation certificates."'
                        ...
                        <div className="flex justify-center">
                          ...
                          >
                            <span className="border-b-2 border-transparent group-hover:border-black pb-1">
                              {locale === 'en' ? 'Log in to Portal' : 'Ingresar al Portal'}
                            </span>
            ...
            <div className="py-20 text-center text-gray-400 italic">
              {locale === 'en' ? 'Agenda to be defined...' : 'Agenda por definir...'}
            </div>
          )}
        </div>
      </div>
