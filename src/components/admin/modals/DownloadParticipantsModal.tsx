      ...
      {isOpen && (
        ...
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-[#373737]">Descargar Base de Datos</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
               <motion.button 
                ...
                className={`p-4 rounded-2xl border-2 transition-colors flex flex-col items-center gap-2 ${dbType === 'organizers' ? 'border-[var(--color-acid)] bg-[rgb(var(--color-acid-rgb)/0.15)] text-black shadow-none' : 'border-gray-100 bg-gray-50 text-gray-500 hover:border-gray-200'}`}
               >
               ...
               <motion.button 
                ...
                className={`p-4 rounded-2xl border-2 transition-colors flex flex-col items-center gap-2 ${dbType === 'users' ? 'border-[var(--color-acid)] bg-[rgb(var(--color-acid-rgb)/0.15)] text-black shadow-none' : 'border-gray-100 bg-gray-50 text-gray-500 hover:border-gray-200'}`}
               >
                 <span className="font-bold text-sm">Participantes</span>
                 <span className="text-[10px] opacity-60">Usuarios registrados</span>
               </motion.button>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
               {[
                 ...
                 { id: 'grado', label: 'Grado Académico' },
                 ...
                 <motion.button
                   ...
                   className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-colors ${fields[f.id as keyof typeof fields] ? 'bg-[rgb(var(--color-acid-rgb)/0.15)] border-[var(--color-acid)] text-black' : 'bg-white border-gray-100 text-gray-500 hover:border-gray-200'}`}
                 >
               ...
               {conference.gives_global_certificate && (
                  <motion.button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-colors ${fields.constanciaGeneral ? 'bg-[rgb(var(--color-acid-rgb)/0.15)] border-[var(--color-acid)] text-black' : 'bg-white border-gray-100 text-gray-500 hover:border-gray-200'}`}
                  >
               ...
               {showCustomFields && customInputs.map(input => (
                  <motion.button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-colors ${customFields[input.id] ? 'bg-[rgb(var(--color-acid-rgb)/0.15)] border-[var(--color-acid)] text-black' : 'bg-white border-gray-100 text-gray-500 hover:border-gray-200'}`}
                  >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Button 
                  ...
                  variant="outline"
                  className="w-full h-14 bg-white border-gray-200 text-[#373737] hover:bg-gray-50 gap-3 rounded-2xl shadow-sm"
                >
