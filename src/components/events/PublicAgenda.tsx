        ...
        <div className="space-y-12">
          {events.length > 0 ? (
            (() => {
              ...
              events.forEach(e => {
                const dateKey = new Date(e.date).toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' });
              ...
              return dateKeys.map((dateKey, dateIdx) => {
                const dayEvents = groupedEvents[dateKey];
                const dateObj = new Date(dateKey + 'T12:00:00'); 
                const locale = language === 'es' ? 'es-MX' : 'en-US';
                
                const dayName = formatMexicoDate(dateObj, { weekday: 'long' }, locale);
                const dateString = formatMexicoDate(dateObj, { day: 'numeric', month: 'long' }, locale);

                return (
                  <div key={dateKey} className="space-y-6">
                    <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-md py-3 border-b border-gray-100 mb-6">
                      <div className="flex items-baseline gap-3">
                        <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tight">
                          {dayName}
                        </h3>
                        <span className="text-lg font-medium text-gray-400 capitalize">
                          {dateString}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {dayEvents.map((e: any, idx: number) => {
                        ...
                        const timeStr = eventDate.toLocaleTimeString(language === 'es' ? 'es-MX' : 'en-US', {
                          hour: '2-digit',
                          minute: '2-digit',
                          hour12: false,
